...
class AzureDevOpsWiki(Wiki):
    ...
    def get_page(self, path):
        ...
        try:
            res = requests.get(
                url=f"{self.url}/pages?includeContent=True&path={path}&api-version=6.0&recursionLevel=OneLevel",
                auth=self.basicAuth,
            )
            ...
            print(f"Error getting page {path} from wiki {self.wikiName}: {e}")
            raise
        return Page(
            name=pageName,
            path=directory,
            content=page["content"],
            childPages=page["subPages"],
        )
...
class ConfluenceWiki(Wiki):
    ...
    def get_page(self, pageTitle):
        requests.get(
            url=f"{self.url}?title={pageTitle}&spaceKey={self.spaceKey}",
            auth=self.basicAuth,
        )
    ...
    def upload_page(self, path, page: Page, attachementDirectory=None):
        ...
        directory, _pageName = self.split_path(path)
        ...
        content = (
            content.replace("<br>", " ").replace("</br>", " ").replace("<pre>", " ")
        )
        ...
        html = markdown.markdown(
            content, extensions=["markdown.extensions.tables", "toc"]
        )
        ...
        try:
            createdPage = self.confluence.create_page(
                space=self.space,
                title=unquotedTitle,
                body=html,
                parent_id=parentId,
            ...
            print(f"Error creating page {unquotedTitle}: {e}")

        for attachment in page.attachments:
            try:
                self.confluence.attach_file(
                    self.attachementDirectory + attachment,
                    page_id=createdPage["id"],
                    title=createdPage["title"],
                    space=self.space,
                )
            except Exception as e:
                print(f"Could not upload attachement {attachment}: {e}")
        print("Page Uploaded!")
